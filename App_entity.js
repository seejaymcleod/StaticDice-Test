        ...
        function toggleArsenalMenu(id, e) {
            ...
            if (!isShowing) {
                ...
                let currentWrapper = parentItem;
                while (currentWrapper) {
                    currentWrapper.classList.add('z-50');
                    currentWrapper = currentWrapper.parentElement ? currentWrapper.parentElement.closest('.arsenal-item-wrapper') : null;
                }
                ...
                if (e && parentItem) {
                    ...
                    // Since the menu is appended to document.body, position absolutely based on page coordinates
                    let pageX = clientX + window.scrollX;
                    let pageY = clientY + window.scrollY;

                    // Bound checks to ensure menu doesn't draw offscreen
                    const menuWidth = 170;
                    if (pageX + menuWidth > window.innerWidth + window.scrollX) {
                        pageX = window.innerWidth + window.scrollX - menuWidth - 8;
                    }
                    if (pageX < 8) pageX = 8;
        ...
        function configureSavedWidget(widgetId, event) {
            ...
            document.getElementById('widget-hide-name').checked = !!q.hideName;

            const typeSelect = document.getElementById('widget-type');
            if (!typeSelect.querySelector(`option[value="${q.widgetType}"]`)) {
                const opt = document.createElement('option');
                opt.value = q.widgetType;
                opt.textContent = q.widgetType.toUpperCase();
                opt.hidden = true;
            ...
            if (document.getElementById('addon-stepper-label')) document.getElementById('addon-stepper-label').value = hasCounter ? q.addonCounter.label : 'Resource';
            if (document.getElementById('addon-stepper-max')) document.getElementById('addon-stepper-max').value = hasCounter ? q.addonCounter.max : '20';
            if (document.getElementById('addon-stepper-val')) document.getElementById('addon-stepper-val').value = hasCounter ? q.addonCounter.value : '20';
            if (document.getElementById('addon-toggle-on')) document.getElementById('addon-toggle-on').value = hasToggle ? q.addonToggle.labelOn : 'Ready';
            if (document.getElementById('addon-toggle-off')) document.getElementById('addon-toggle-off').value = hasToggle ? q.addonToggle.labelOff : 'Miscast';
            if (document.getElementById('addon-note-text')) document.getElementById('addon-note-text').value = q.addonNote || '';
            ...
            if (document.getElementById('stepper-min')) document.getElementById('stepper-min').value = q.min !== undefined ? q.min : '0';
            if (document.getElementById('stepper-max')) document.getElementById('stepper-max').value = q.max !== undefined ? q.max : '20';
            if (document.getElementById('stepper-val')) document.getElementById('stepper-val').value = q.value !== undefined ? q.value : '20';
            if (document.getElementById('stepper-tracker')) document.getElementById('stepper-tracker').checked = !!q.showTracker;

            // Populate standalone toggle configs
            if (document.getElementById('toggle-label-on')) document.getElementById('toggle-label-on').value = q.labelOn || 'On';
            if (document.getElementById('toggle-label-off')) document.getElementById('toggle-label-off').value = q.labelOff || 'Off';
            if (document.getElementById('toggle-initial')) document.getElementById('toggle-initial').checked = !!q.checked;

            // Populate text card configs
            if (document.getElementById('text-card-content')) document.getElementById('text-card-content').value = q.text || '';

            // Populate number config
            if (document.getElementById('number-val')) document.getElementById('number-val').value = q.value !== undefined ? q.value : '10';
            ...
            if (q.widgetType === 'timer') {
                if (document.getElementById('timer-max-min')) document.getElementById('timer-max-min').value = Math.floor((q.maxTime || 600) / 60);
                if (document.getElementById('timer-curr-min')) document.getElementById('timer-curr-min').value = Math.floor((q.currentTime ?? 600) / 60);
                if (document.getElementById('timer-curr-sec')) document.getElementById('timer-curr-sec').value = (q.currentTime ?? 600) % 60;
                if (document.getElementById('timer-dice-formula')) document.getElementById('timer-dice-formula').value = q.diceFormula || '2d6';
                if (document.getElementById('timer-rundown-text')) document.getElementById('timer-rundown-text').value = q.rundownText || "Time's Up!";
                if (document.getElementById('timer-animation')) document.getElementById('timer-animation').value = q.animationType || 'none';
            }

            // Populate entity group config
            if (q.widgetType === 'entity-group') {
                if (document.getElementById('entity-group-prefix')) document.getElementById('entity-group-prefix').value = q.entityTemplate?.namePrefix || 'Entity';
                if (document.getElementById('entity-group-hp')) document.getElementById('entity-group-hp').value = q.entityTemplate?.baseHp !== undefined ? q.entityTemplate.baseHp : 10;
            }

            // Populate detail text
            if (document.getElementById('widget-detail-text')) document.getElementById('widget-detail-text').value = q.detailText || '';
            ...
            if (document.getElementById('widget-full-formula')) document.getElementById('widget-full-formula').checked = q.fullShowFormula !== false;
            if (document.getElementById('widget-full-note')) document.getElementById('widget-full-note').checked = q.fullShowNote !== false;
            if (document.getElementById('widget-full-detail')) document.getElementById('widget-full-detail').checked = q.fullShowDetail !== false;
            if (document.getElementById('widget-show-formula')) document.getElementById('widget-show-formula').checked = q.showFormula !== false;
            if (document.getElementById('widget-show-note')) document.getElementById('widget-show-note').checked = q.showNote !== false;
            if (document.getElementById('widget-show-detail')) document.getElementById('widget-show-detail').checked = q.showDetail !== false;
            if (document.getElementById('widget-compact-formula')) document.getElementById('widget-compact-formula').checked = !!compactShowFormula;
            if (document.getElementById('widget-compact-note')) document.getElementById('widget-compact-note').checked = !!compactShowNote;
            if (document.getElementById('widget-compact-detail')) document.getElementById('widget-compact-detail').checked = !!compactShowDetail;

<<<<<<< Updated upstream
=======
            // Populate grid & layout config
            if (document.getElementById('widget-parent-id')) document.getElementById('widget-parent-id').value = q.parentId || '';
            if (document.getElementById('widget-colspan')) document.getElementById('widget-colspan').value = q.colSpan !== undefined ? q.colSpan : 12;

            // Populate trigger config
            if (q.widgetType === 'trigger') {
                if (document.getElementById('trigger-target-id')) document.getElementById('trigger-target-id').value = q.targetWidgetId || '';
                if (document.getElementById('trigger-condition')) document.getElementById('trigger-condition').value = q.condition || '<=';
                if (document.getElementById('trigger-val')) document.getElementById('trigger-val').value = q.conditionValue !== undefined ? q.conditionValue : 0;
                if (document.getElementById('trigger-action')) document.getElementById('trigger-action').value = q.action || 'show-button';
                if (document.getElementById('trigger-btn-label')) document.getElementById('trigger-btn-label').value = (q.actionParams && q.actionParams.label) || '☠️ Kill';
                if (document.getElementById('trigger-btn-color')) document.getElementById('trigger-btn-color').value = (q.actionParams && q.actionParams.btnColor) || 'rose';
            }
>>>>>>> Stashed changes
...
=======
            ...
            document.getElementById('widget-entity-group-config')?.classList.add('hidden');
>>>>>>> Stashed changes
...
=======
                ...
                document.getElementById('widget-entity-group-config')?.classList.remove('hidden');
>>>>>>> Stashed changes
            }
        ...
        async function submitWidgetCreation() {
            ...
            if (editingWidgetId !== null) {
                ...
                if (q) {
                    q.name = name;
                    q.hideName = document.getElementById('widget-hide-name').checked;
                    q.widgetType = type;
...
<<<<<<< Updated upstream
=======
                    const parentEl = document.getElementById('widget-parent-id');
                    q.parentId = parentEl ? parentEl.value || null : q.parentId;
                    const colSpanEl = document.getElementById('widget-colspan');
                    q.colSpan = colSpanEl ? parseInt(colSpanEl.value, 10) || 12 : q.colSpan;

                    if (type === 'entity-group') {
                        if (!q.entityTemplate) q.entityTemplate = { widgets: [] };
                        q.entityTemplate.namePrefix = document.getElementById('entity-group-prefix').value.trim() || 'Entity';
                        
                        const baseHp = parseInt(document.getElementById('entity-group-hp').value, 10) || 10;
                        q.entityTemplate.baseHp = baseHp;
                        
                        // Ensure basic template structure exists
                        let hpTpl = q.entityTemplate.widgets.find(w => w.widgetType === 'stepper');
                        if (!hpTpl) {
                            hpTpl = {
                                id: 'w_tpl_' + Date.now() + '_hp_' + Math.random().toString(36).substring(2, 7),
                                name: 'HP',
                                widgetType: 'stepper',
                                min: 0,
                                max: baseHp,
                        ...
                        if (!q.entityTemplate.widgets.some(w => w.widgetType === 'text')) {
                            q.entityTemplate.widgets.push({
                                id: 'w_tpl_' + Date.now() + '_note_' + Math.random().toString(36).substring(2, 7),
                                name: 'Notes',
                                widgetType: 'text',
                                displayMode: 'micro',
                                colSpan: 12
...
=======
            ...
            const colSpanEl = document.getElementById('widget-colspan');
            const colSpan = colSpanEl ? parseInt(colSpanEl.value, 10) || 12 : 12;
...
>>>>>>> Stashed changes
            let newWidget = {
                ...
                name: name,
                hideName: document.getElementById('widget-hide-name').checked,
                color: color,
                widgetType: type,
                queue: [],
            ...
            if (type === 'entity-group') {
                ...
                newWidget.entityTemplate = {
                    namePrefix: document.getElementById('entity-group-prefix').value.trim() || 'Entity',
                    baseHp: baseHp,
                    widgets: [
                        {
                            id: 'w_tpl_' + Date.now() + '_hp_' + Math.random().toString(36).substring(2, 7),
...
=======
        ...
        function spawnGroupEntity(groupWidgetId) {
            ...
            tplWidgets.forEach((tplWidget, idx) => {
                idMap[tplWidget.id] = 'w_child_' + Date.now() + '_' + idx + '_' + Math.random().toString(36).substring(2, 5);
            });

            tplWidgets.forEach((tplWidget) => {
                const childId = idMap[tplWidget.id];
                const childWidget = JSON.parse(JSON.stringify(tplWidget));
                childWidget.id = childId;
                
                if (tplWidget.parentId && idMap[tplWidget.parentId]) {
                    childWidget.parentId = idMap[tplWidget.parentId];
                } else {
